...
<div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="d-flex">
                    <h4 class="font-semibold text-xl text-gray-800 leading-tight">Edit Data</h4>
                </div>
                <form action="{{ route('alat.update', $alat->id)}}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="nama_alat" value="{{ $alat->nama_alat }}" required>
                    </div>
                    <div class="form-group mt-3">
                        <label for="deskripsi">Deskripsi</label>
                        <input type="text" class="form-control" name="deskripsi" value="{{ $alat->deskripsi }}" required>
                    </div>
                    <div class="form-group mt-3">
                ...
                <div class="form-group">
                            ...
                            <select name="id_kategori" class="form-control" required>
                                @foreach ($kategori as $k)
                                    <option value="{{ $k->id }}" {{ $alat->id_kategori == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama_kategori }}
                                    </option>
                                @endforeach
                            </select>
